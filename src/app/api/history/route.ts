...
export async function POST(req:Request){try{sameOrigin(req);await requireAdmin();if(Number(req.headers.get('content-length')||0)>20000000)return Response.json({error:'File exceeds 20 MB'},{status:413});const b=await req.json();return Response.json(await importHistory(b.records,b.filename||'data/historic-tickets.json',{close:b.close!==false}));}catch(e){return errorResponse(e);}}
