"use client";
import {useEffect,useCallback,useMemo,useState} from 'react';import {CalendarDays,Ticket,Clock3,CheckCircle2,TriangleAlert,ArrowUpRight,MapPin,MessageSquare,Sparkles,Flame,Hash} from 'lucide-react';
...
export type KanbanGroupBy='status'|'priority'|'category'|'owner'|'studio'|'department';
export type KanbanField='member'|'studio'|'department'|'owner'|'status'|'priority'|'category'|'source'|'age'|'sla';
const DEFAULT_KANBAN_FIELDS:KanbanField[]=['member','status','priority','category','owner','sla'];
...
export function TicketTable({tickets,onSelect,selected,onToggle}:{tickets:TicketListRecord[];onSelect?:(id:number)=>void;selected?:number[];onToggle?:(id:number)=>void}){
  ...
  return <div className="table-wrap"><table className="data-table data-table-rich"><thead><tr>{onToggle&&<th style={{width:15}}/>}<th>TICKET & MEMBER</th><th>CLASSIFICATION</th><th>STATUS</th><th>PRIORITY</th><th>OWNER</th><th style={{textAlign:'right'}}>SLA TIMER</th></tr></thead><tbody>{tickets.map(t=>{const age=ageOf(t);return <tr key={t.id} className={'priority-row priority-row-'+t.priority} onClick={()=>open(t.id)}>
    ...
    <td><div className={'mini-owner'+(!t.assignedStaffName?' mini-owner-empty':'')}><Avatar name={t.assignedStaffName||'Unassigned owner'} tone="purple" emptyDark/><span><strong>{t.assignedStaffName?.split(' ')[0]||'Unassigned'}</strong><small>{t.departmentName||SOURCE_LABEL[t.source]||'—'}</small></span></div></td>
    <td><SlaCountdown ticket={t}/></td></tr>;})}</tbody></table></div>;}

export function TicketCard({ticket:t,onSelect,fields=DEFAULT_KANBAN_FIELDS}:{ticket:TicketListRecord;onSelect?:(id:number)=>void;fields?:KanbanField[]}){const age=ageOf(t);const has=(f:KanbanField)=>fields.includes(f);return <button className={'ticket-card priority-edge priority-edge-'+t.priority} onClick={()=>onSelect?onSelect(t.id):window.location.assign('/tickets/'+t.id)}>
  <div className="between"><span className="accent mono" style={{fontSize:10}}>{t.ticketNumber}</span>{has('priority')&&<Priority priority={t.priority}/>}</div>
  <h3>{t.title}</h3>
  {has('member')&&<div className="ticket-card-who"><Avatar name={t.memberName||'Member'}/><div className="grow"><strong>{t.memberName}</strong><small>{has('studio')?t.studio?.split(',')[0]:''}{has('department')?(has('studio')?' · ':'')+String(t.departmentName||'—'):''}</small></div></div>}
  <div className="ticket-chips">
    {has('category')&&<span className="chip chip-kind" data-tone={KIND_TONE[t.kind]||''}>{t.category}</span>}
    {has('source')&&<span className="chip chip-quiet">{SOURCE_LABEL[t.source]||t.source}</span>}
    {has('age')&&<span className={'chip chip-quiet'+(age.stale?' chip-warn':'')}>{age.label}</span>}
  </div>
  <div className="between ticket-card-foot">{has('status')&&<Status status={t.status}/>}{has('owner')&&<span className="ticket-card-owner" title={t.assignedStaffName||'Unassigned'}><Avatar name={t.assignedStaffName||'Unassigned owner'} tone="purple" emptyDark/><small>{t.assignedStaffName?.split(' ')[0]||'Queue'}</small></span>}</div>
  {has('sla')&&<SlaCountdown ticket={t}/>} 
  {age.stale&&<span className="ticket-flag"><Flame size={9}/>ageing</span>}
</button>;}
export function Kanban({tickets,onSelect,groupBy='status',fields=DEFAULT_KANBAN_FIELDS}:{tickets:TicketListRecord[];onSelect?:(id:number)=>void;groupBy?:KanbanGroupBy;fields?:KanbanField[]}){
  const statusColumns=[{id:'new',label:'Incoming',states:['new','triaged']},{id:'assigned',label:'Assigned',states:['assigned']},{id:'in_progress',label:'In progress',states:['in_progress']},{id:'waiting',label:'Awaiting response',states:['waiting_on_member','waiting_on_vendor']},{id:'done',label:'Completed',states:['resolved','closed','recorded']}];
  const grouped=useMemo(()=>{
    if(groupBy==='status')return statusColumns.map(c=>({id:c.id,label:c.label,rows:tickets.filter(t=>c.states.includes(t.status))}));
    const keyOf=(t:TicketListRecord)=>groupBy==='priority'?t.priority:groupBy==='category'?t.category:groupBy==='owner'?(t.assignedStaffName||'Unassigned'):groupBy==='studio'?(t.studio?.split(',')[0]||'No studio'):(t.departmentName||'No department');
    const map=new Map<string,TicketListRecord[]>();
    for(const t of tickets){const key=keyOf(t);map.set(key,[...(map.get(key)||[]),t]);}
    return Array.from(map.entries()).sort((a,b)=>b[1].length-a[1].length).map(([key,rows])=>({id:key.toLowerCase().replace(/\s+/g,'-'),label:key,rows}));
  },[tickets,groupBy]);
  return <div className="kanban">{grouped.map(c=>{const rows=c.rows;return <div className="kanban-column" key={c.id}><div className="kanban-head"><div><strong style={{fontWeight:600}}>{c.label}</strong><small className="kanban-sub">{rows.filter(t=>['high','critical'].includes(t.priority)).length} urgent · {rows.filter(t=>t.slaDueAt&&slaState(t.slaDueAt,t.status)==='breached').length} overdue</small></div><Badge>{rows.length}</Badge></div>{rows.map(t=><TicketCard key={t.id} ticket={t} onSelect={onSelect} fields={fields}/>)}{!rows.length&&<p className="muted" style={{fontSize:11,padding:15,textAlign:'center'}}>All clear here.</p>}</div>;})}</div>;}
